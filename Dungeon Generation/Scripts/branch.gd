...
# FIXED: Build connection map after rooms are created with improved logic
func build_connections():
	var corridors = get_corridors()
	var leaves = get_leaves()
	...
	print("Building connections with %d corridors for %d rooms" % [corridors.size(), leaves.size()])
	
	# Build connections based on corridors with improved logic
	for corridor in corridors:
		var start_pos = corridor['start']
		var end_pos = corridor['end']
		
		print("Processing corridor from (%d,%d) to (%d,%d)" % [start_pos.x, start_pos.y, end_pos.x, end_pos.y])
		
		# FIXED: Find rooms that contain or are adjacent to corridor endpoints
		...
		if room1 != null and room2 != null and room1 != room2:
			...
			if not room1.connected_rooms.has(room2):
				room1.connected_rooms.append(room2)
				print("  Connected room at (%d,%d) with room at (%d,%d)" % 
					[room1.room_top_left.x, room1.room_top_left.y, room2.room_top_left.x, room2.room_top_left.y])
			if not room2.connected_rooms.has(room1):
				room2.connected_rooms.append(room1)
		else:
			if room1 == null:
				print("  Warning: Could not find room for start position (%d,%d)" % [start_pos.x, start_pos.y])
			if room2 == null:
				print("  Warning: Could not find room for end position (%d,%d)" % [end_pos.x, end_pos.y])
			if room1 == room2 and room1 != null:
				print("  Warning: Both corridor endpoints are in the same room")

# FIXED: Improved room finding for corridor endpoints
...
# FIXED: Consistent boundary checking 
...
# NEW: Calculate distance from point to room
...
# FIXED: More reliable _get_child_center logic
func _get_child_center(node: Branch) -> Vector2i:
	# Direct room center if this node has a room
	if node.has_room:
		return node.get_room_center()
	
	# For non-leaf nodes, find the best representative center
...
# Helper function to find which room contains a specific point (kept for compatibility)
func _find_room_containing_point(point: Vector2i) -> Branch:
	if has_room and _point_in_room_bounds(point, self):
		return self
...
# Check if this room has a neighbor of a specific type
func has_neighbor_of_type(target_type: RoomGrammar.RoomType, all_rooms: Array[Branch], current_types: Array[RoomGrammar.RoomType]) -> bool:
	for connected_room in connected_rooms:
		# Find the index of this connected room in the all_rooms array
		var room_index = all_rooms.find(connected_room)
